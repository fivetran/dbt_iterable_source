...
## Bug Fixes
- Explicitly cast the `email_list_ids` field in the `stg_iterable__user_history` model to be a string. This ensures the JSON as string operations in the downstream models are not impacted by errors if the connector syncs this field as a JSON datatype. ([PR #29](https://github.com/fivetran/dbt_iterable_source/pull/29))
