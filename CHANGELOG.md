...
## Test Updates
- The unique tests in `stg_iterable__event` and `stg_iterable__event_extension` now tests for a unique combination of `event_id` and `unique_user_key`. Previously it was just `event_id`, but now that `event_id` isn't the unique key, we're leveraging `unique_user_key`, which is either `_fivetran_user_id` if it exists, or email. `_fivetran_user_id` is a key available in the Aug 2023 Iterable API that's generated by taking hash of user_id and/or email fields depending on the project type.
