...
- Removes `created_at` from the uniqueness test for `stg_iterable__event`. Uniqueness is tested solely on `unique_event_id`, a surrogate key made up of `event_id` (`_fivetran_id` in the raw table, which is a Fivetran-created unique identifier derived from hashing campaign_id, created_at, and event_name) and `_fivetran_user_id`.
