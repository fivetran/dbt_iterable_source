...
- New connectors will have `user_unsubscribed_channel` and `user_unsubscribed_message_type`, no longer syncing the history versions. As such we removed the `*_history` from `stg_iterable__user_unsub_message_type`, `stg_iterable__user_unsubscribed_channel`, `stg_iterable__user_unsub_message_type_tmp`, and `stg_iterable__user_unsubscribed_channel_tmp`.
...
- Added the field `_fivetran_id` to `stg_iterable__user_history`
- Introduced passthrough column functionality for `user_history` and `event` objects. For more information refer to the README(/README.md)
...
- Past August 2023, the `user_unsubscribed_channel_history` and `user_unsubscribed_message_type_history` Iterable objects will no longer be history tables. In addition, the columns have changed. We have checks in place that will automatically persist the respective columns depending on what exists in your schema.
  - In addition, we have added generated keys to these 2 tables that we will use for uniqueness tests. The unique key generated is either `_fivetran_id` or `email`, depending on what exists in your tables (determined by whether you are using the history version). For more information, refer to our [documentation](https://github.com/fivetran/dbt_iterable_source/blob/main/models/stg_iterable.yml).
